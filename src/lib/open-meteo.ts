...
export module OpenMeteo {
    export const Url = new URL("https://api.open-meteo.com/v1/");

    export enum WeatherInterpretationCodes {
        CLEAR_SKY = 0,
    ...
    export interface Daily<Time extends TimeFormat> {
    ...
    export type Result<
        H extends keyof Hourly = never,
        D extends keyof Daily<Time> = never,
        Time extends TimeFormat = TimeFormat.ISO,
    ...
    > = RemoveNeverKeys<{
        ...
        hourly: [H] extends [never]
            ? never
            : { time: TimeFromFormat<Time> } & Record<H, Array<number>>;
        hourly_units: [H] extends [never] ? never : { [key in H]: Hourly[key] };
        daily: [D] extends [never]
            ? never
            : { time: TimeFromFormat<Time> } & Record<D, Array<number>>;
        daily_units: [D] extends [never]
            ? never
            : { [key in D]: Daily<Time>[key] };
        current_weather: Current extends true ? CurrentWeather<Time> : never;
    }>;

    export interface CurrentWeather<Time extends TimeFormat> {
    ...
    export type TimeFromFormat<T extends TimeFormat> = T extends TimeFormat.ISO
        ? string
        : number;

    export type PastDays = 0 | 1 | 2;

    export interface Input<
        H extends keyof Hourly,
        D extends keyof Daily<Time>,
        Time extends TimeFormat,
        Current extends boolean = false
    > {
        latitude: number;
    ...
    export class API extends Pariah {
        ...
        public async forecast<
            H extends keyof Hourly = never,
            D extends keyof Daily<T> = never,
            T extends TimeFormat = TimeFormat.ISO,
            Current extends boolean = false
        >(
            body: Input<H, D, T, Current>
        ): Promise<Data<Result<H, D, T, Current>>> {
            const newBody = {
                ...body,
